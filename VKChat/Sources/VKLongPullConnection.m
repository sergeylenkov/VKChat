...
- (void)connectToServer {
    ...
    pull = [[ASIHTTPRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://%@?act=a_check&key=%@&ts=%ld&wait=25&mode=%ld", _server, _key, _ts, _mode]]] retain];
    ...
    [pull setCompletionBlock:^() {
        ...
        if (!response) {
            ...
            if ([response objectForKey:@"failed"]) {
                if (showLog) {
                    NSLog(@"PULL ERROR %@", response);
                }
