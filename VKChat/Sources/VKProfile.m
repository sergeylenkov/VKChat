...
- (void)_initWithDictionary:(NSDictionary *)dict withPhoto:(BOOL)withPhoto {
    self.identifier = [[dict objectForKey:@"uid"] intValue];
	...
	self.photoBigURL = [NSURL URLWithString:[dict objectForKey:@"photo_big"]];
	self.photoRecURL = [NSURL URLWithString:[dict objectForKey:@"photo_rec"]];
    ...
    if ([[dict objectForKey:@"city"] intValue] > 0) {
        __block VKGetCityByIdRequest *request = [[VKGetCityByIdRequest alloc] init];
        
        request.accessToken = [VKAccessToken token];
        request.cityID = [[dict objectForKey:@"city"] intValue];
        
        VKGetCityByIdRequestResultBlock resultBlock = ^(NSString *name) {
            self.city = name;
            [request release];
        };
