...
- (void)setDialog:(VKDialog *)aDialog {
    ...
    if (dialog.isChat) {
        key = [NSString stringWithFormat:@"%ld", dialog.chatID];
    } else {
...
- (void)updateTypingInChat:(NSNotification *)notification {
    if (dialog.isChat && [[notification.object objectForKey:@"chat"] intValue] == dialog.chatID) {
        dialog.isOnline = YES;
        isUserTyping = YES;
        typingUserID = [[notification.object objectForKey:@"user"] intValue];
