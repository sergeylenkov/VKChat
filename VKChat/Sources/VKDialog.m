...
- (id)initWithDictionary:(NSDictionary *)dict {
    ...
    if (self) {
        ...
        if ([dict objectForKey:@"chat_id"]) {
            self.isChat = YES;
            self.chatID = [[dict objectForKey:@"chat_id"] intValue];
            self.users = [[dict objectForKey:@"chat_active"] componentsSeparatedByString:@","];
        }
...
- (void)receivedNewMessage:(NSNotification *)notification {
	...
	if (self.isChat && [dict objectForKey:@"chat"]) {
		if (self.chatID != [[dict objectForKey:@"chat"] integerValue]) {
			return;
		}
	}
