...
- (void)refreshView {
    ...
    for (int i = 0; i < [_filteredDialogs count]; i++) {
        ...
        if ([defaults objectForKey:@"SelectedDialogChatID"] && dialog.isChat) {
			if (dialog.chatID == [defaults integerForKey:@"SelectedDialogChatID"]) {
				selectedRow = i;
			}
        }
...
- (BOOL)isDialogExists:(VKDialog *)dialog {
	for (VKDialog *_dialog in _dialogs) {
		if (dialog.isChat && _dialog.isChat) {
			if (dialog.chatID == _dialog.chatID) {
				return YES;
			}
		}
...
- (void)listViewSelectionDidChange:(NSNotification *)aNotification {
    ...
    if (row != -1) {
        if (row < [_filteredDialogs count]) {
            ...
            if (dialog.isChat) {
                [defaults setInteger:dialog.chatID forKey:@"SelectedDialogChatID"];
